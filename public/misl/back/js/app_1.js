...
$('body').on('click', '[stack-search]', function(){
	...
	if (rURL) {
		$.ajax({
			type: 'post',
		    url: rURL,
		    data:{stack_token:stack_token, name:name, region:region, district:district, township:township,},
		    dataType: "json",
		    beforeSend: function(){

		    },
		    success: function(results){
		    	$('#stack-data-set').html(results.html);

        ...
        <div class="col-lg-12">
            <div class="ibox ">
                ...
                <div class="ibox-content">
                    <form action="{{ route('carmodelUpdate', $carmodel->id) }}" method="POST" accept-charset="utf-8" enctype="multipart/form-data">
                    @csrf
                        <div class="form-group row {{ $errors->has('name') ? 'has-error':'' }}">
                            <label class="col-sm-2 col-form-label"><?php echo translate('car_model_name'); ?></label>
                            <div class="col-sm-10">
                                <input type="text" name="name" value="{{ old('name', $carmodel->name) }}" placeholder="<?php echo translate('placeholder_car_model_name') ?>"  class="form-control" >
                            ...
                            <div class="col-sm-10">
                                <select  name="brand_id" class="form-control single_select2" data-live-search="true"  placeholder="<?php echo translate('placeholder_car_brand') ?>" >
                                    ...
                                    @foreach($brands as $brand)
                                        <option value="{{ $brand->id }}" <?php echo ($brand->id == $carmodel->brand_id)? 'selected': ''; ?> >{{ $brand->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group row {{ $errors->has('seo_title') ? 'has-error':'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" name="seo_title" placeholder="<?php echo translate('placeholder_seo_title') ?>" class="form-control" value="{{ old('seo_title', $carmodel->seo_title) }}">

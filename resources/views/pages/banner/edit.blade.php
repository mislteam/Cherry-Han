        ...
        <div class="col-lg-12">
            <div class="ibox">
                ...
                <div class="ibox-content">
                    <form action="{{ route('bannerUpdate', $banner->id) }}" method="POST" accept-charset="utf-8" enctype="multipart/form-data">
                    @csrf
                        <div class="form-group row {{ $errors->has('page_name') ? 'has-error':'' }}">
                            ...
                            <div class="col-sm-8">
                                <input type="text" name="page_name" value="<?php echo $banner->page_name; ?>" placeholder="<?php echo translate('placeholder_page_name') ?>"  class="form-control">
                            ...
                            <div class="col-sm-8">
                                <select  name="service_type" class="form-control select2_demo_1" placeholder="@lang('form.car.country_id.placeholder')">
                                    ...
                                    @foreach($services as $service)
                                        <option value="{{ $service->type }}" <?php if ($service->type==$banner->service_type): echo "selected"; ?>
                                            
                                        <?php endif ?>>{{ $service->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row {{ $errors->has('banner_type') ? 'has-error':'' }}">
                            ...
                            <div class="col-sm-8">
                                <input type="text" name="banner_type" value="<?php echo $banner->banner_type; ?>" placeholder="<?php echo translate('placeholder_banner_type') ?>"  class="form-control">

        ...
        <div class="col-lg-12">
            <div class="ibox col-lg-6 mx-auto">
                <div class="ibox-title">
                    <h5><?php echo translate('edit_car_type'); ?></h5>
                    @can('car.view')
                ...
                <div class="ibox-content">
                    <form action="{{ route('cartypeUpdate', $cartype->id) }}" method="POST" accept-charset="utf-8" enctype="multipart/form-data">
                    @csrf
                        <div class="form-group row {{ $errors->has('name') ? 'has-error':'' }}">
                            <label class="col-sm-2 col-form-label"><?php echo translate('car_name_*'); ?></label>
                            <div class="col-sm-10">
                                <input type="text" name="name" value="{{ $cartype->name }}" placeholder="<?php echo translate('placeholder_car_name') ?>"  class="form-control">
                        ...
                        <div class="form-group row {{ $errors->has('service_type') ? 'has-error':'' }}">
                            ...
                            <div class="col-sm-10">
                                <select  name="service_type" class="form-control select2_demo_1" placeholder="<?php echo translate('placeholder_service_type') ?>" >
                                    <option><?php echo translate('choose_service_type'); ?></option>
                                    @foreach($services as $service)
                                    <option value="{{ $service->type }}"  <?php echo ($cartype->service_type == $service->type)?'selected':''; ?>>{{ $service->name }}</option>
                                    @endforeach
                                </select>

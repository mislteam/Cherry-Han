                ...
                <div class="ibox-content">
                    <form action="{{ route('roleUpdate',$role->id) }}" method="post">
                        ...
                        <div class="form-group row {{ $errors->has('name') ? 'has-error':'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" name="name" placeholder="<?php echo translate('placehoder_role_name') ?>" class="form-control" value="{{ old('name', $role->name) }}">
                            ...
                            <div class="col-sm-10">
                                <select  name="permissions[]" id="permission" class="multi_select2 form-control " multiple="multiple" data-live-search="true"  placeholder="<?php echo translate('placeholder_permission') ?>" required>
                                    @foreach($permissions as $permission)
                                        <option value="{{ $permission->name }}" {{ ($role->hasPermissionTo($permission->name) || $role->id ==1) ? 'selected':'' }} >{{ $permission->title   }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group row {{ $errors->has('title') ? 'has-error':'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" name="title"  placeholder="<?php echo translate('placehoder_role_title') ?>" class="form-control" value="{{ old('title', $role->title) }}">
